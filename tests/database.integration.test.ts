...
it("partage les fiches publiques sans exposer Pathbuilder ni les notes privées", async () => {
  ...
  await db.exec(`select public.update_my_player_page('${campaign.campaign_id}', 'Aster', 'La Sentinelle écarlate', 'Présentation A', 'https://pathbuilder2e.com/app.html', 'Secret A', 'Objectif A', null, 35, 62, 1.75)`);
  const playerPages = (await db.query<{ user_id: string; is_own: boolean; character_title: string | null; pathbuilder_url: string | null; notes: string | null; image_x: number; image_y: number; image_zoom: number }>(`select user_id, is_own, character_title, pathbuilder_url, notes, image_x::float, image_y::float, image_zoom::float from public.list_campaign_player_pages('${campaign.campaign_id}') order by user_id`)).rows;
  expect(playerPages).toHaveLength(2);
  expect(playerPages.find((page) => page.user_id === first)).toMatchObject({ is_own: true, character_title: 'La Sentinelle écarlate', pathbuilder_url: 'https://pathbuilder2e.com/app.html', notes: 'Secret A', image_x: 35, image_y: 62, image_zoom: 1.75 });
