...
it("propose seulement les deux boutons MJ et Public dans le journal", async () => {
  await act(async () => { root.render(<GmApp campaignId={DEMO_CAMPAIGN_ID} />); });
  const journalButton = await waitFor(() => buttonContaining("Journal"));
  ...
  const addButton = await waitFor(() => buttonContaining("Ajouter"));
  act(() => addButton.click());
