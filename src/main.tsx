...
if (!import.meta.env.DEV) {
  ...
  window.addEventListener("input", markInteraction, { once: true, capture: true });
  void fetch(`/build-version.json?check=${Date.now()}`, { cache: "no-store" })
    ...
    .then((release) => {
      ...
      if (interacted || url.searchParams.get("v") === release.version) return;
      url.searchParams.set("v", release.version);
      window.location.replace(url.toString());
    })
    .catch(() => undefined)
    .finally(() => window.removeEventListener("input", markInteraction, true));
}
