...
export function sanitizeContent(value: string): string {
  ...
  return DOMPurify.sanitize(body.innerHTML, {
    ALLOWED_TAGS: Array.from(allowed, (tag) => tag.toLowerCase()),
    ALLOW_DATA_ATTR: false,
    ADD_ATTR: ["data-session-id", "data-locked", "data-callout-id", "data-indent", "data-session-date", "contenteditable", "target", "face", "size"],
  });
}
...
export function QuestWritingTab({ page, revisions = [], canRestoreHistory = false, demo, onChanged, onNotice, onError }: Props) {
  ...
  useEffect(() => {
    ...
    if (canvas && canvas.innerHTML !== page.content) { canvas.innerHTML = sanitizeContent(page.content); normalizeCanvas(canvas); }
  ...
  useEffect(() => { const canvas = canvasRef.current; if (canvas && document.activeElement !== canvas && canvas.innerHTML !== content) { canvas.innerHTML = sanitizeContent(content); normalizeCanvas(canvas); } }, [content]);
