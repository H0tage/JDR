...
function EconomyActivity({ data, saving, onExecute }: { data: PlayerEconomyData; saving: boolean; onExecute: (operation: () => Promise<unknown>, success: string) => Promise<void> }) {
  ...
  return <section className="economy-activity panel"><header><div><History size={18} /><h2>Journal des opérations</h2></div><input type="search" value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Joueur, objet, commentaire…" /></header><div>{entries.length ? entries.map((row) => <article key={`${row.kind}-${row.entry.id}`}><time>{formatDateTime(row.date)}</time><div><strong>{describeActivity(row.entry)}</strong>{row.entry.comment && <small>{row.entry.comment}</small>}</div>{row.kind === "money" && canCancelMoney(row.entry, data.viewer_user_id) && <button className="button tiny secondary" disabled={saving} onClick={() => void onExecute(() => cancelMoneyTransaction(row.entry.id), "Opération annulée.")}>Annuler</button>}{row.kind === "item" && row.entry.event_type === "sold" && row.entry.actor_user_id === data.viewer_user_id && <button className="button tiny secondary" disabled={saving} onClick={() => void onExecute(() => cancelItemEvent(row.entry.id), "Vente annulée.")}>Annuler la vente</button>}</article>) : <p className="empty-state">Aucune opération correspondante.</p>}</div></section>;
}
