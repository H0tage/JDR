...
function PlayerPagesTab({ page, loading, error, viewerRole, theme }: { page: CampaignPlayerPage | null; loading: boolean; error: string | null; viewerRole: "gm" | "player"; theme: PlayerTheme }) {
  ...
  return <div className="page-stack player-page-tab player-pages-readonly">
    ...
    <div className="player-character-sheet">
      <section className="player-character-hero player-character-hero-readonly">{portrait ? <button type="button" className="player-character-portrait player-character-portrait-button" onClick={() => setPortraitOpen(true)} title="Agrandir l’illustration"><img src={portrait} style={playerPortraitCropStyle(page)} alt={page.character_name ? `Illustration de ${page.character_name}` : `Illustration de ${page.display_name}`} /><span><Maximize2 size={16} />Agrandir</span></button> : <div className="player-character-portrait"><div><UserRound size={48} /><span>Aucune illustration</span></div></div>}<div className="player-character-intro"><h1>{page.character_name || "Personnage sans nom"}</h1>{page.character_title && <p className="player-character-title">{page.character_title}</p>}<p>{page.character_summary || "Aucune présentation renseignée."}</p></div></section>
...
function emptyPlayerPageDraft(page: PlayerPage): PlayerPageDraft {
  return {
    ...
    image_x: page.image_x ?? 50,
    image_y: page.image_y ?? 50,
    image_zoom: page.image_zoom ?? 1,
  };
}

function playerPortraitCropStyle(page: Pick<PlayerPage, "image_x" | "image_y" | "image_zoom">): CSSProperties {
  const zoom = page.image_zoom ?? 1;
  const x = page.image_x ?? 50;
  const y = page.image_y ?? 50;
  return {
...
function PlayerPageTab({ campaignId, demo, active, playerPages, theme }: { campaignId: string; demo: boolean; active: boolean; playerPages: CampaignPlayerPage[]; theme: PlayerTheme }) {
  ...
  function chooseImage(file: File | null) {
    ...
    if (file) setDraft((current) => current ? { ...current, image_x: 50, image_y: 50, image_zoom: 1 } : current);
  }
  ...
  return <div className={`page-stack player-page-tab${pathbuilderFocused ? " pathbuilder-focus-mode" : ""}`}>
    ...
    <div className="player-page-register-view" hidden={pageView !== "register"}><p className="player-page-privacy"><LockKeyhole size={17} /><span><strong>Votre fiche de campagne.</strong> Les autres joueurs peuvent la consulter en lecture seule. Vos notes privées restent visibles uniquement par vous et le MJ.</span></p>{editing ? <form className="player-page-edit" onSubmit={save}>
      <section className="player-page-edit-portrait"><div className="player-page-portrait-preview">{portrait ? <img src={portrait} style={playerPortraitCropStyle(draft)} alt="Aperçu du personnage" /> : <ImagePlus size={40} />}</div><label className="button secondary player-page-image-picker"><ImagePlus size={16} />Choisir ou remplacer l’image<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => chooseImage(event.target.files?.[0] ?? null)} /></label>{portrait && <div className="contact-crop-controls player-page-crop-controls"><strong>Cadrage de l’illustration</strong><label>Zoom<input aria-label="Zoom du portrait" type="range" min="1" max="2.5" step="0.05" value={draft.image_zoom} onChange={(event) => setDraft({ ...draft, image_zoom: Number(event.target.value) })} /></label><label>Centrage horizontal<input aria-label="Centrage horizontal du portrait" type="range" min="0" max="100" step="1" value={draft.image_x} onChange={(event) => setDraft({ ...draft, image_x: Number(event.target.value) })} /></label><label>Centrage vertical<input aria-label="Centrage vertical du portrait" type="range" min="0" max="100" step="1" value={draft.image_y} onChange={(event) => setDraft({ ...draft, image_y: Number(event.target.value) })} /></label><button type="button" className="text-button" onClick={() => setDraft({ ...draft, image_x: 50, image_y: 50, image_zoom: 1 })}>Réinitialiser le cadrage</button></div>}{(draft.image_path || imageFile) && <button type="button" className="text-button danger-text" onClick={() => { chooseImage(null); setDraft({ ...draft, image_path: null, image_x: 50, image_y: 50, image_zoom: 1 }); }}>Retirer l’image</button>}<small>JPEG, PNG ou WebP · 8 Mo maximum.</small></section>
      <section className="player-page-edit-fields"><label>Nom du personnage<input maxLength={120} value={draft.character_name ?? ""} onChange={(event) => setDraft({ ...draft, character_name: event.target.value })} placeholder="Nom utilisé en jeu" /></label><label>Titre du personnage <small>facultatif</small><input maxLength={160} value={draft.character_title ?? ""} onChange={(event) => setDraft({ ...draft, character_title: event.target.value })} placeholder="Ex. La Flèche rouge de Féérune" /></label><label>Présentation<textarea maxLength={4000} value={draft.character_summary ?? ""} onChange={(event) => setDraft({ ...draft, character_summary: event.target.value })} placeholder="Concept, parcours et éléments importants…" /></label><label>Lien Pathbuilder<input type="url" maxLength={500} value={draft.pathbuilder_url ?? ""} onChange={(event) => setDraft({ ...draft, pathbuilder_url: event.target.value })} placeholder="https://pathbuilder2e.com/…" /></label></section>
...
function PlayerRelationshipCard({ campaignId, demo, page, initialNotes, onSaved }: { campaignId: string; demo: boolean; page: CampaignPlayerPage; initialNotes: string; onSaved: (targetUserId: string, notes: string) => void }) {
  ...
  return <article className="player-party-relation-card">
    <header><span className="contact-portrait">{portrait ? <img src={portrait} style={playerPortraitCropStyle(page)} alt="" /> : <UserRound size={19} aria-hidden="true" />}</span><div><small>{page.display_name}</small><h3>{page.character_name || "Personnage non renseigné"}</h3></div></header>
