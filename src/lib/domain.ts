...
/**
 * Returns the most demanding service the group can genuinely request now.
 * Unlike the reputation-only unlock list, this also accounts for current
 * favours, tension and any personalised service costs.
 */
export function highestAvailableService(
  services: Service[],
  faction: { slug: string; rp: number; jf: number; tension: number },
  settings: CampaignSettings,
): ServiceScale | null {
  return [...services]
    .sort((left, right) => scaleRank[right.scale] - scaleRank[left.scale])
    .find((service) => quoteService(service, faction, settings).allowed)?.scale ?? null;
}
