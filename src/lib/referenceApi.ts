...
import { anonymizeDemoArchives, anonymizeDemoLoot } from "../data/demoAnonymization";
...
export async function loadArchives(campaignId: string, demo = false): Promise<ArchivesData> {
  if (demo) {
    return anonymizeDemoArchives(archiveCharacterSeeds, archivePlaceSeeds);
  }
...
export async function loadLoot(campaignId: string, demo = false): Promise<LootEntry[]> {
  if (demo) return anonymizeDemoLoot(lootSeeds);
...
export async function setLootPlayerVisibility(id: string, playerVisible: boolean): Promise<void> {
  const result = await requireClient().rpc("set_loot_player_visibility", {
    p_loot_id: id,
    p_visible: playerVisible,
    p_published_on: localCalendarDate(),
  });
  check(result.error, "Visibilité du butin");
}

/** YYYY-MM-DD dans le fuseau du navigateur : la date de publication est locale. */
