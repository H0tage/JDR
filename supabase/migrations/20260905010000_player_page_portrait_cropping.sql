-- Nom conseillé dans le SQL Editor : Cadrage des portraits joueurs — 2026-09-05
-- Conserve un cadrage non destructif du portrait et le partage avec toutes les vues autorisées.
...
alter table public.player_pages
  drop constraint if exists player_pages_image_x_range,
  drop constraint if exists player_pages_image_y_range,
  drop constraint if exists player_pages_image_zoom_range,
  add constraint player_pages_image_x_range check (image_x between 0 and 100),
  add constraint player_pages_image_y_range check (image_y between 0 and 100),
  add constraint player_pages_image_zoom_range check (image_zoom between 1 and 2.5);
...
-- Compatibilité avec les clients déjà ouverts : ils conservent le cadrage existant.
...
returns table(
  ...
  is_own boolean, character_name text, character_summary text,
  pathbuilder_url text, notes text, objectives text, updated_at timestamptz,
  image_path text, image_x numeric, image_y numeric, image_zoom numeric
)
